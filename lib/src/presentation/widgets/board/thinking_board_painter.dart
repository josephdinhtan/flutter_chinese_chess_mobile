...
class ThinkingBoardPainter extends CustomPainter {
  ...
  void drawMoveLine(Canvas canvas, int from, int to, int index) {
    ...
    final fx = left +
        (layoutParams.boardInverse ? 8 - fc : fc) * layoutParams.squareWidth;
    final fy = top +
        (layoutParams.boardInverse ? 9 - fr : fr) * layoutParams.squareWidth;

    double tx = left +
        (layoutParams.boardInverse ? 8 - tc : tc) * layoutParams.squareWidth;
    double ty = top +
        (layoutParams.boardInverse ? 9 - tr : tr) * layoutParams.squareWidth;
